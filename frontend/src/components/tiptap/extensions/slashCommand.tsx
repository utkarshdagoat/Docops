...
const OptionList  = ({items , command}:{items:CommandItemProps[] , command:any} )=>{
  ...
  return items.length > 0 ? (
    <div 
      className="flex flex-col rounded-md bg-white shadow-3xl items-center max-h-[225px] overflow-scroll focus:outline-none " 
      id="popup"
      ref={divRef}
    >
      ...
      {items.map((item : CommandItemProps, index: number)=>{
        return (
          <button 
            key={index} 
            className={`flex items-center px-1.5 py-1 min-w-full space-x-3 ${bgColor(index)} hover:bg-slash-hover`}
